...
CUDAArray<int> constructBuckets(T* similaritiesDatabase, int templatesNumber, const LUT_C &templateTags, short databaseHeight) {
	...
	CUDAArray<int> bucketMatrix(quantLevels, templatesNumber);
	cudaStatus = cudaMemset2D(bucketMatrix.cudaPtr, bucketMatrix.Stride, 0, bucketMatrix.Width * sizeof(int), bucketMatrix.Height);
...
__global__ void quantify(CUDAArray<float> src, CUDAArray<int> dst, LUT_C templateTags) {
	size_t x = defaultColumn(),
		y = defaultRow();

	__shared__ int subBucket[2][quantLevels];
	
	// auxilary indices for simultaneous access to subBucket
	int currentRow = threadIdx.y / 2;
	int currentColumn = threadIdx.x + (threadIdx.y % 2) * blockDim.x;

	if (currentRow < 2)
		subBucket[currentRow][currentColumn] = 0;

	__syncthreads();

	if ((x < src.Width) && (y < src.Height)) {
		// row in subBucket coresponding to current thread
		short localRow = templateTags[x] - templateTags[x - threadIdx.x];

		int level = (int)(quantLevels * src.At(y, x));
		atomicAdd(subBucket[0] + localRow * quantLevels + level, 1);
	}

	__syncthreads();

	if (currentRow < 2)
		atomicAdd(dst.cudaPtr + (templateTags[x - threadIdx.x] + currentRow) * (dst.Stride / sizeof(int)) + currentColumn, subBucket[currentRow][currentColumn]);
}
...
int main() {
	...
	float* linDB = linearizeDoublePointer(DB, 0, queryTemplateSize, width);*/
	...
	fin_database >> width >> queryTemplateSize;

	linDB = new float[width * queryTemplateSize];

	for (int i = 0; i < width; ++i)
	for (int j = 0; j < queryTemplateSize; ++j)
		fin_database >> linDB[j * width + i];
