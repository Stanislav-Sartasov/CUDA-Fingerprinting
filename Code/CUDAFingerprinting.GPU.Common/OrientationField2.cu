...
// возвращает сумму произведений соответствующих элементов квадратных матриц a и b
// матрицы a и b поступают как линеаризованные массивы
__device__ float SumOfProducts(CUDAArray<float> a, CUDAArray<float> b){
	...
	product.SetAt(threadRow, threadColumn, product.At(threadRow, threadColumn) * b.At(row, column)); // копируем в общую память произведения соответствующих элементов 
	__syncthreads();  // ждем пока все нити сделают перемножение

	// теперь нужно просуммировать элементы этой матрицы
	...
	// после прохода циклов сумма сумм будет находиться в product.At(0, 0)
	return product.At(0, 0);
}



// возвращает сумму разностей квадратов элементов массивов a и b
...
__global__ void cudaSetOrientation(CUDAArray<float> orientation, CUDAArray<float> gradientX, CUDAArray<float> gradientY){
	...
	numerator = 2 * SumOfProducts(gradientX, gradientY);		// удвоенная сумма произведений соответствующих элементов
	denominator = SumOfSquaresDifference(gradientX, gradientY);	// сумма разностей квадратов элеметов матриц
